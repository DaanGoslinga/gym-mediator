...
def main():
    '''
    loads and visualizes the learned policy
    '''
    env = gym.make("Mediator-v1")
    act = learn(
        env,
        network=models.mlp(num_layers=3, num_hidden=128, activation=tf.tanh, layer_norm=False),
        total_timesteps=0,
        load_path="mediator_model.pkl"
    )
    ...
    while True:
        ...
        while not done: 
            ...
            d_att_now = int(obs[0][0])
            if action ==-10:
                action = act(sub_obs[None])[0]
                action = med_libs.action_Q_env(action) # TODO: map to env_action
            
            if d_att_now==1:
                print('action', list(mydict.keys())[list(mydict.values()).index(action)])

            obs, rew, done, _ = env.step(action)
            sub_obs = med_libs.custom_obs(obs)
            episode_rew += rew
